...
def chunk_fts(q_im,u_im,clims_pix_y,clims_pix_x,width,ft_taper,pad,i, beam):
...
def binning(ee_scaled, bb_scaled, ell_r, bins, area_width):    
...
#Plot the APS top left
    ...
    im1=ax2.imshow(np.log(np.abs(ee_scaled))[256:768,256:768])
    ...
    im2=ax3.imshow(np.log(np.abs(bb_scaled))[256:768,256:768])
...
for i in range (nochunks):
    ee_scaled, bb_scaled = chunk_fts(q_im, u_im, clims_pix_y, clims_pix_x, width, ft_taper, pad, i, beam)

    ee_average, nonzero_ee, bb_average, nonzero_bb, eb, nz = binning(ee_scaled, bb_scaled, ell_r, bins, area_width)
