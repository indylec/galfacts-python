...
v_hdu=fits.open(v_in) 
v_im=v_hdu[0].data

if len(v_im.shape) == 3:
    v_im=v_im[0,:,:]

v_head=v_hdu[0].header
...
v_chunk=v_im[clims_pix_y[i]:clims_pix_y[i]+width,clims_pix_x[i]:clims_pix_x[i]+width]
#u_chunk=u_im[clims_pix_y[i]:clims_pix_y[i]+width,clims_pix_x[i]:clims_pix_x[i]+width]

#remove mean

v_chunk=v_chunk-np.mean(v_chunk)
#u_chunk=u_chunk-np.mean(u_chunk)

#Do FTs
#taper, pad
v_chunk=np.pad(v_chunk*ft_taper,(pad,),mode='constant')
#u_chunk=np.pad(u_chunk*ft_taper,(pad,),mode='constant')

#ft
vft=np.fft.fftshift(np.fft.fft2(v_chunk))
#uft_final=np.fft.fftshift(np.fft.fft2(u_chunk))

vft=vft/(1024**2.)
#vft_final=vft/ft_beam
#output 2d image

plt.imshow(np.log(np.abs(vft)))
plt.colorbar()
plt.show()
plt.savefig('/Users/leclercq/galfacts/inpainting/'+field+'_2dft_V_'+str(i)+'.png',dpi=200)
