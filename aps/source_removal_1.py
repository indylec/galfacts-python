...
qtbdata=tbdata[tbdata['Peak_2']>rms_factor*tbdata['rms_2']]
...
#get pixel coords of source center: convert from RA,DEC: use astropy WCS functionality
    temp_pixel=w_sky_q.wcs_world2pix([[tabline['RA_2a'],tabline['DEC_2a']]])
...
#this equation tells you which pixels are contained within an ellipse with a,b,theta and centered on xc and yc (all in pixels, theta in rad)
    inside=np.where((((x-xc_temp)*np.cos(theta)-(y-yc_temp)*np.sin(theta_temp))**2/a_temp**2+((x-xc_temp)*np.sin(theta_temp)+(y-yc_temp)*np.cos(theta_temp))**2/b_temp**2)<1)
